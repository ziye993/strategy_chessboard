import { IBlockPoint, TColorArray, TPoint, TVertexs } from "@/type/game.type";
...
export function drawHexagon(ctx: CanvasRenderingContext2D, center: IBlockPoint, color: TColorArray, content = 0, fill = 0, changeTip?: string, tipTop?: number) {
  ...
  changeTip && tipTop && ctx.fillText(changeTip, center.x, tipTop)
...
*/
export function clearHexagonByPoints(ctx: CanvasRenderingContext2D, points: TVertexs) {
  if (points.length !== 6) {
 ...
 */
export function drawCrosshair(ctx: CanvasRenderingContext2D, x: number, y: number, color = 'red') {
  // 保存当前上下文状态
  ctx.save();
...
//线条
export function drawLine(ctx: CanvasRenderingContext2D, points: TVertexs, startColor: TColorArray, endColor: TColorArray, lineWidth = 0.4) {
  // 验证参数
...
// 已知斜边时，求长直角边的长度
function getSideLingth(size: number) {
...
export function getVertexs({ x, y, size }: TPoint & { size: number }) {
...
//计算中心点
export function getHexagonCenters(wl: number, hl: number, size: number) {
  const sideLength = getSideLingth(size);
  const centers: TCenters = [];
  let padding = 100;
  ...
  return { centers, maxWidth, maxHeight, padding }
}
